...
class ForgotPassword extends React.Component {
    ...
    getEmail = () => {
        window.apiHelper.user.forgotPassword(this.state.email, (res) => {
            if(res) this.setState({validationEmail: true, step: 2});
            else this.setState({validationEmail: 'Wrong email address.'})
        })
    };
            ...
            <BottomContainer>
                {this.state.step == 1 ?
                    <>
                        ...
                        <BB_Button label='Send me new password' onClick={this.getEmail}/>
                    </>
                    :
                    <>
